...
# Main Screen
form = Tk()
form.title('Banking Application')
form.config(padx=60, pady=60)
    ...
    for name in all_accounts:
        if name == login_name:
            ...
            if login_password == password:
                ...
                account_dashboard = Toplevel(form)
...
def finish_withdraw():
    ...
    if float(withdraw_amount.get()) > float(current_balance):
        withdraw_notify.config(text='Insufficient Funds!', fg='red')
        return
...
def personal_details():
    ...
    personal_details_screen = Toplevel(form)
...
# Labels
Label(form, text="Online Banking", font=('Calibri', 14)).grid(row=0, sticky=N, pady=10)
Label(form, image=img).grid(row=2, sticky=N, pady=15)

# Buttons
Button(form, text="Register", font=('Calibri', 12), width=20, command=register).grid(row=3, sticky=N)
Button(form, text="Login", font=('Calibri', 12), width=20, command=login).grid(row=4, sticky=N, pady=10)
